...
func (m *Emailer) SendMail(mailTo string, title, content string) error {
	fromUrl := "127.0.0.1"
	appName := language.Obj.Get("common.app_name")
	mUser := repository.User{Mail: mailTo}
        ...
        <td style="width: 600px;padding: 18px;display: inline-block;">
            <div align="rignt">
                <div style="font-size: 14px; margin: 0px;text-align: right;font-size: 14px; font-weight: bolder;">
                    -- ` + language.Obj.Get("mail.from") + `[<a href="` + fromUrl + `" style="color: #575757;">` + appName + `</a>]</div>
            </div>
        </td>
    </tr>
</table>`
...
func (m *Emailer) SendMailOfRegister(mailTo, key string) error {
	fromUrl := "127.0.0.1"
	appName := language.Obj.Get("common.app_name")
	title := language.Obj.GetWithFields("mail.register_title", map[string]string{
		"AppName": appName,
	})
	content := language.Obj.GetWithFields("mail.register_content", map[string]string{
		"AppName": appName,
	})
	return m.SendMailOfLink(mailTo, title, content, language.Obj.Get("mail.register_click_btn"), fromUrl+"/profile/auth.html#/linkRegister?code="+key)
}
